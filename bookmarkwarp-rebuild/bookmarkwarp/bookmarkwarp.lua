...
-- init Frame.
function BookmarkWarp.InitFrame(addon, frame)
  ...
  BookmarkWarp.addon = addon;
  BookmarkWarp.frame = frame;
  frame:ShowWindow(0);
  frame:EnableMove(1);
  frame:SetEventScript(ui.LBUTTONDOWN, "BookmarkWarp_StartDrag");
  frame:SetEventScript(ui.LBUTTONUP, "BookmarkWarp_EndDrag");
end

--start drag
function BookmarkWarp_StartDrag(addon, frame)
  BookmarkWarp.isDragging = true;
end

-- end drag.
function BookmarkWarp_EndDrag(addon, frame)
  local frame = ui.GetFrame('bookmarkwarp');
  BookmarkWarp.isDragging = false;
...
function BookmarkWarp.CreateButton(i, warpName, warpcost)
  ...
  local button = frame:CreateOrGetControl("button", "warpButton_"..warpName, 10, (i-1)*48+10, 300, 32);
  ...
  -- if warpcost > -1 then
	...
	-- if usedItem == true then
	  ...
	  -- button:SetText(string.sub(mapCls.Name, 1, 25)..". . .")
	-- else
	...
	if warpcost > -1 then
		button:SetEnable(1);
		button:SetText(mapCls.Name);
		local realText = button:GetText()
		button:SetTextTooltip(realText);
		if usedItem == false then
			button:SetTextTooltip(realText.." {img silver 20 20}"..warpcost);
			if #realText > 19 then
				transedText = string.sub(realText,1,19)..". . ."
				button:SetText(transedText.." {img silver 20 20}"..warpcost);
			else
				button:SetText(realText.." {img silver 20 20}"..warpcost);
			end
		elseif #realText > 30 then
			button:SetText(string.sub(realText,1,30)..". . .");
		end
    ...
    -- button:SetText(mapCls.Name);
    -- if usedItem == false then
      -- local transedText = button:GetText()
      -- if #transedText > 20 then
          -- transedText = string.sub(transedText,1,20) .. "..."
        -- button:SetText(transedText.." {img silver 20 20}"..warpcost);
      -- end
    -- end
