...
function STATUSPOINTCHECK_WHIGHTSQUESTCHECK(weight)
	...
	for k, questNo in pairs(questList) do
		...
		sumPoint = sumPoint + tonumber(pcProperty.Value);
		nameBody = questCls.Name;
		pointBody = "Weight +"..pcProperty.Value
		if STATUSPOINTCHECK_QUESTCLEARCHECK(questNo) then
			color = "{#FF3333}{ol}{b}{s16}";
			resultBody = "OK"
			mapBody = ""
...
function STATUSPOINTCHECK_ZEMINACLEARCHECK(mapClassName)
	local npcGenTypes = GetNPCStateGenTypes(mapClassName);
	if not npcGenTypes then
		return false;
	end
	for i = 1, #npcGenTypes do
		local genCls = GetGenTypeClass(mapClassName, npcGenTypes[i]);
		if genCls and genCls.ClassType == "statue_zemina" then
			local npcState = GetNPCState(mapClassName, genCls.GenType);
			return npcState == 20;
		end
	end
	return false;
